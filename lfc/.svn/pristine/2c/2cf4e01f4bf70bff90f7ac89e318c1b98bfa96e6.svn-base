...
app.controller('AppController', ['$scope', '$rootScope', '$http', '$timeout', 'PATH','messageService','$location',function ($scope, $rootScope, $http, $timeout, PATH,messageService,$location) {
    ...
    $rootScope.$on('$stateChangeSuccess',function(){
        ...
        if($location.$$path=='/login'){
            ...
            if(typeof getMsg == 'undefined'){
                msgList();
                getMsg=setInterval(msgList,60000);
            }
        }
    });
