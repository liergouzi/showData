...
app.controller('playController', ['$scope', function ($scope) {
    ...
    $scope.clearopacity = function () {
        var eachths=$('.ea chths');
        eachths.find('ul').addClass('ths');
        var thead=$('.digital table thead');
        var ths=thead.children()[0];
        var childs=$(ths).children();
        var lastth=childs[childs.length-2];
        $(lastth).find('ul').addClass('lastth').removeClass('ths');
    ...
    $scope.sitetables = {
        ...
        tbodys:[
            ...
            {week: '站点10',Sales:'480', percentage:'26.55%', Dsales: '312', Dpercentage: '17.26%', redsales: '396', redpercentage: '21.90%', bulesales: '60',   bluepercentage:'3.32%',  Qsales: '480 ',  Qpercentage:'26.55%', Tsales: '80', Tpercentage: '4.42%', weeksum: '1808'},
        ],
